    ...
    try {
      ...
      if (!response.ok) {
        try {
          ...
          setError(
            `Server error ${response.status}: Please check if resend-verify is uploaded to the server.`,
          );
        }
        return;
      }
      ...
      if (authMode === "login") {
        ...
        if (!response.ok) {
          try {
            ...
            setError(
              `Server error ${response.status}: Please check if reset is uploaded.`,
            );
          }
          return;
        }
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
                ...
                {devResetLink && (
                  <div className="bg-blue-50 border border-blue-100 text-blue-800 px-4 py-4 rounded-xl text-xs flex flex-col items-center justify-center gap-2 text-center">
                    ...
                    <p className="text-blue-600">
                      Email sending is skipped. Click the button below to test
            ...
            {authMode !== "forgot" && authMode !== "reset" && (
              <div className="flex items-center px-1">
                ...
                <label
                  htmlFor="remember-me"
                  className="ml-3 block text-xs font-bold text-slate-600 cursor-pointer"
                >
                  Keep me logged in
                </label>
              </div>
