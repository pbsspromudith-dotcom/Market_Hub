...
const BuyAndSellToronto: React.FC = () => {
  ...
  useEffect(() => {
    fetch('/api/listings/read?location=Toronto')
      .then(res => res.json())
      .then(data => setListings(Array.isArray(data) ? data.filter((l: any) => !['Jobs', 'Real Estate'].some(c => l.category?.startsWith(c))).slice(0, 8) : []))
      .catch(console.error);
  }, []);
    ...
    <div className="overflow-x-hidden">
      <section className="bg-gradient-to-br from-emerald-900 via-emerald-700 to-teal-500 text-white py-20 md:py-28 relative overflow-hidden">
        ...
        <div className="w-full px-4 sm:px-6 lg:px-10 relative z-10 text-center">
          ...
          <Link href="/search" className="bg-white text-emerald-700 font-black px-10 py-5 rounded-2xl hover:bg-slate-50 transition-all shadow-xl text-sm uppercase tracking-widest inline-block">
            Start Shopping
          </Link>
        </div>
      </section>
        ...
        <section className="mb-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <Link key={cat.name} href={`/search?q=${encodeURIComponent(cat.name)}`} className="bg-white p-6 rounded-2xl border border-slate-100 text-center hover:border-emerald-500 hover:shadow-lg transition-all group">
