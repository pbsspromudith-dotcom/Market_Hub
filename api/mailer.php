...
function sendPasswordResetEmail($toEmail, $userName, $token) {
    ...
    $subject = "Reset Your Password — HitAds.ca";
    $html = buildResetPasswordHtml($userName, $token);
    return $mailer->send($toEmail, $subject, $html);
}
 ...
 */
function buildResetPasswordHtml($userName, $token) {
    $year = date('Y');
    $safeName = htmlspecialchars($userName);
    $resetLink = "https://hitads.ca/login?mode=reset&token=" . urlencode($token);
    ...
    <tr>
      <td align="center">
        <table width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff; border-radius:24px; overflow:hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.06);">
          ...
          <tr>
            <td style="padding: 48px 40px;">
              <h2 style="margin:0 0 8px; color:#1a1a1a; font-size:24px; font-weight:900;">Reset Your Password, {$safeName}! 🔑</h2>
              <p style="margin:0 0 28px; color:#64748b; font-size:15px; line-height:1.7;">
                We received a request to reset the password for your account on <strong>HitAds.ca</strong>. Please click the button below to choose a new password. This link is valid for 1 hour.
              </p>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center">
                    <a href="{$resetLink}" style="display:inline-block; background-color:#cc2d2d; color:#ffffff; text-decoration:none; padding:16px 48px; border-radius:50px; font-size:14px; font-weight:900; text-transform:uppercase; letter-spacing:2px;">
                      Reset My Password
                    </a>
                  </td>
                </tr>
          ...
          <tr>
            <td style="padding: 24px 40px 32px; border-top: 1px solid #f1f1f1; text-align:center;">
              <p style="margin:0 0 4px; color:#94a3b8; font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:2px;">© {$year} HitAds.ca — All rights reserved.</p>
              <p style="margin:0; color:#cbd5e1; font-size:11px;">You received this email because a password reset was requested for your account.</p>
            </td>
          </tr>
